  ...
  template: `
    <div class="min-h-screen bg-cyber-bg p-4 md:p-8 text-white font-sans">
      ...
      <div class="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
        <div>
          <h1 class="text-3xl font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-cyber-primary to-cyber-secondary">
            SYSTEM MANAGEMENT
          </h1>
        </div>
        <button (click)="goBack()" 
      ...
      <div class="flex gap-2 mb-6 overflow-x-auto pb-2">
        <button (click)="switchTab('tracks')" 
                [class]="activeTab === 'tracks' ? 'bg-cyber-primary text-black shadow-[0_0_15px_rgba(0,243,255,0.4)]' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white'"
        ...
        <button (click)="switchTab('schools')" 
                [class]="activeTab === 'schools' ? 'bg-cyber-primary text-black shadow-[0_0_15px_rgba(0,243,255,0.4)]' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white'"
                class="px-6 py-3 rounded-t-lg font-bold text-sm tracking-wider transition-all border-b-2 border-transparent flex items-center gap-2 whitespace-nowrap">
          🏫 SCHOOLS
        </button>
      ...
      <div *ngIf="activeTab === 'tracks'" class="bg-gray-900/60 border border-cyber-primary/30 rounded-xl rounded-tl-none overflow-hidden backdrop-blur-md shadow-lg p-4 animate-fadeIn">
        ...
        <div class="bg-black/40 border border-white/10 p-4 rounded-xl mb-6 shadow-inner">
           ...
           <div class="flex flex-col lg:flex-row gap-4 items-start">
              <div class="grid grid-cols-2 md:grid-cols-4 gap-2 w-full">
                 <input type="text" [(ngModel)]="newTrack.trackCode" placeholder="Code (e.g. AI)" class="bg-gray-900 border border-white/20 rounded px-3 py-2 text-sm text-white focus:border-cyber-primary outline-none">
                 <input type="text" [(ngModel)]="newTrack.trackNameTh" placeholder="Name (TH)" class="bg-gray-900 border border-white/20 rounded px-3 py-2 text-sm text-white focus:border-cyber-primary outline-none">
                 <input type="text" [(ngModel)]="newTrack.trackNameEn" placeholder="Name (EN)" class="bg-gray-900 border border-white/20 rounded px-3 py-2 text-sm text-white focus:border-cyber-primary outline-none">
                 <input type="text" [(ngModel)]="newTrack.description" placeholder="Description" class="bg-gray-900 border border-white/20 rounded px-3 py-2 text-sm text-white focus:border-cyber-primary outline-none">
              </div>
              <button (click)="addTrack()" [disabled]="!newTrack.trackCode || !newTrack.trackNameTh" class="bg-cyber-primary text-black px-6 py-2 rounded-lg font-bold text-xs uppercase tracking-wider hover:scale-105 transition-all shadow-[0_0_15px_rgba(0,243,255,0.3)] disabled:opacity-50 h-[38px] flex items-center gap-2 whitespace-nowrap">
                 <span>+</span> Add
              </button>
           </div>
        </div>
             ...
             <tbody class="divide-y divide-white/5">
                <tr *ngFor="let t of tracks" class="hover:bg-white/5 transition-colors">
                   <ng-container *ngIf="editingTrackId !== t.trackId; else editTrackMode">
                      ...
                      <td class="p-3 text-xs leading-relaxed max-w-md text-gray-400">{{t.description || '-'}}</td>
                      <td class="p-3 text-center">
                        <button *ngIf="!isDeleteMode" (click)="startEditTrack(t)" class="group text-cyber-secondary hover:text-white transition-all p-2 rounded-full hover:bg-cyber-secondary/20" title="Edit">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                        </button>
                        <button *ngIf="isDeleteMode" (click)="confirmDeleteTrack(t)" class="bg-red-500/20 text-red-500 border border-red-500/50 p-2 rounded-full hover:bg-red-600 hover:text-white transition-all shadow-[0_0_10px_rgba(239,68,68,0.4)]" title="DELETE">
                   ...
                   <ng-template #editTrackMode>
                      ...
                      <td class="p-3"><input [(ngModel)]="tempTrackData.trackNameEn" class="bg-black/50 border border-cyber-primary p-2 rounded w-full text-white text-xs focus:ring-1 focus:ring-cyber-primary outline-none"></td>
      ...
      <div *ngIf="activeTab === 'schools'" class="bg-gray-900/60 border border-cyber-primary/30 rounded-xl rounded-tl-none overflow-hidden backdrop-blur-md shadow-lg p-4 animate-fadeIn">
        
        <div class="bg-black/40 border border-white/10 p-4 rounded-xl mb-6 shadow-inner">
          <div class="flex flex-col xl:flex-row gap-6">
            <div class="flex-1">
              <label class="text-[10px] text-gray-400 uppercase tracking-wider mb-2 block font-bold flex items-center gap-1">🔍 Find School</label>
              <div class="relative">
                <input type="text" [(ngModel)]="schoolSearchTerm" placeholder="Type school name..." 
                ...
                <span class="absolute right-3 top-3 text-gray-500 text-xs" *ngIf="schoolSearchTerm">Found {{ (filteredSchools | slice:0:50).length }}</span>
              </div>
            </div>
        ...
        <div class="overflow-x-auto max-h-[500px] custom-scrollbar">
          <table class="w-full text-left text-sm text-gray-300">
            ...
            <tbody class="divide-y divide-white/5">
              <tr *ngFor="let s of filteredSchools | slice:0:50" class="hover:bg-white/5 transition-colors group">
                <td class="p-3 text-center text-gray-600 text-xs">{{ s.id }}</td>
                <ng-container *ngIf="editingSchoolId !== s.id; else editSchoolMode">
                  <td class="p-3 text-white font-medium"><span [innerHTML]="highlightText(s.name, schoolSearchTerm)"></span></td>
                  <td class="p-3 text-gray-400">{{ s.province }}</td>
                  <td class="p-3 text-center">
                    <button *ngIf="!isDeleteMode" (click)="startEditSchool(s)" class="group text-cyber-secondary hover:text-white transition-all p-2 rounded-full hover:bg-cyber-secondary/20" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                    </button>
                    <button *ngIf="isDeleteMode" (click)="confirmDeleteSchool(s.id, s.name)" class="bg-red-500/20 text-red-500 border border-red-500/50 p-2 rounded-full hover:bg-red-600 hover:text-white transition-all shadow-[0_0_10px_rgba(239,68,68,0.4)]" title="DELETE">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                    </button>
                  </td>
                </ng-container>
      ...
      <div *ngIf="activeTab === 'questions'" class="bg-gray-900/60 border border-cyber-primary/30 rounded-xl rounded-tl-none overflow-hidden backdrop-blur-md shadow-lg p-4 animate-fadeIn">
        
        <div class="bg-black/40 border border-white/10 p-6 rounded-xl mb-6 shadow-inner relative overflow-hidden group">
           <div class="absolute -right-10 -top-10 w-32 h-32 bg-cyber-primary/10 blur-[50px] rounded-full group-hover:bg-cyber-primary/20 transition-all"></div>

           <label class="text-[12px] text-cyber-primary uppercase tracking-wider mb-4 font-bold flex items-center gap-2 border-b border-white/5 pb-2">
           ...
           <div class="flex flex-col gap-6">
              ...
              <div class="flex gap-4">
                 <div class="w-24 shrink-0">
                    ...
                    <input type="number" [(ngModel)]="newQuestion.order" class="w-full bg-gray-900 border border-white/20 rounded-lg px-3 py-3 text-center text-white font-bold focus:border-cyber-primary focus:ring-1 focus:ring-cyber-primary outline-none transition-all">
                 </div>
                 <div class="flex-grow">
                    <label class="text-[10px] text-gray-400 uppercase mb-1 block pl-1">Question Text</label>
                    <input type="text" [(ngModel)]="newQuestion.text" placeholder="Enter question description..." class="w-full bg-gray-900 border border-white/20 rounded-lg px-4 py-3 text-white focus:border-cyber-primary focus:ring-1 focus:ring-cyber-primary outline-none transition-all placeholder-gray-600">
                 </div>
              </div>
              ...
              <div class="bg-white/5 rounded-lg p-4 border border-white/5">
                 ...
                 <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div *ngFor="let code of allTrackCodes" class="relative group/input">
                       ...
                       <input type="number" [(ngModel)]="newQuestion.weights[code]" placeholder="0"
        ...
        <div class="flex justify-between items-center mb-4">
           <h3 class="text-cyber-primary font-bold uppercase tracking-wider text-sm border-l-4 border-cyber-primary pl-2">Adjust Scoring Weights</h3>
           <button (click)="toggleDeleteMode()" 
                   [class]="isDeleteMode ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.6)] animate-pulse' : 'bg-gray-800 text-gray-400 border border-gray-600 hover:bg-gray-700'"
                   class="px-4 py-2 rounded-full font-bold text-xs uppercase tracking-wider transition-all flex items-center gap-2">
             <span *ngIf="!isDeleteMode">🗑 Enter Delete Mode</span>
          ...
          <table class="w-full text-left text-sm text-gray-300">
             <thead class="bg-gray-800 uppercase text-xs">
                <tr>
                  ...
                  <th *ngFor="let code of allTrackCodes" class="p-3 text-center text-cyber-secondary w-20">{{ code }}</th>
                  <th class="p-3 text-center w-24">{{ isDeleteMode ? 'DELETE' : 'EDIT' }}</th>
                </tr>
             </thead>
             <tbody class="divide-y divide-white/5">
                <tr *ngFor="let q of questions" class="hover:bg-white/5 transition-colors group">
                   <ng-container *ngIf="editingQuestionId !== q.questionId; else editQMode">
                      ...
                      <td *ngFor="let code of allTrackCodes" class="p-3 text-center">
                         <span [class.text-green-400]="(q.weights[code] || 0) > 0" class="font-mono font-bold">{{ q.weights[code] || 0 }}</span>
                      </td>
                      <td class="p-3 text-center">
                        <button *ngIf="!isDeleteMode" (click)="startEditQuestion(q)" class="group text-cyber-secondary hover:text-white transition-all p-2 rounded-full hover:bg-cyber-secondary/20" title="Edit">
                           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                        </button>
                        <button *ngIf="isDeleteMode" (click)="confirmDeleteQuestion(q)" class="bg-red-500/20 text-red-500 border border-red-500/50 p-2 rounded-full hover:bg-red-600 hover:text-white transition-all shadow-[0_0_10px_rgba(239,68,68,0.4)]" title="DELETE">
                           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                        </button>
                      </td>
                   </ng-container>
                   <ng-template #editQMode>
                      <td class="p-3"><input type="number" [(ngModel)]="tempQuestionData.questionOrder" class="w-12 bg-black/50 border border-cyber-primary rounded text-center text-white"></td>
...
export class AdminManageComponent implements OnInit {
  ...
  addTrack() {
    this.adminService.createTrack(this.newTrack).subscribe({
      next: () => {
        Swal.fire({ title: 'Track Added!', icon: 'success', timer: 1500, showConfirmButton: false, background: '#1a1a1a', color: '#fff' });
  ...
  startEditTrack(t: Track) { this.editingTrackId = t.trackId; this.tempTrackData = { ...t }; }
  cancelEditTrack() { this.editingTrackId = null; }
  saveTrack(id: number) {
  ...
  // --- SCHOOLS ---
  loadSchools() {
    this.adminService.getAllSchools().subscribe(res => { this.schools = res || []; this.cdr.detectChanges(); });
  }
  ...
  confirmAddSchool() {
    this.adminService.createSchool({ name: this.newSchoolName, province: this.newSchoolProv }).subscribe(() => {
        this.newSchoolName = ''; this.newSchoolProv = ''; this.schoolSearchTerm = '';
        this.loadSchools();
        Swal.fire({ title:'Added!', icon:'success', timer: 1500, showConfirmButton: false, background: '#1a1a1a', color: '#fff' });
    });
  }
  ...
  startEditSchool(s: School) { this.editingSchoolId = s.id; this.tempSchoolData = { ...s }; }
  cancelEditSchool() { this.editingSchoolId = null; }
  ...
  loadQuestions() {
    this.adminService.getAllQuestionsAdmin().subscribe(res => {
      ...
      const maxOrder = this.questions.length > 0 ? Math.max(...this.questions.map(q => q.questionOrder)) : 0;
  ...
  addQuestion() {
    this.adminService.createQuestion({
      ...
      next: () => {
        Swal.fire({ title: 'Added!', icon: 'success', timer: 1000, showConfirmButton: false, background: '#1a1a1a', color: '#fff' });
        this.newQuestion.text = ''; this.newQuestion.weights = {};
        this.loadQuestions();
      },
  ...
  startEditQuestion(q: AdminQuestion) { this.editingQuestionId = q.questionId; this.tempQuestionData = { ...q, weights: { ...q.weights } }; }
  cancelEditQuestion() { this.editingQuestionId = null; this.tempQuestionData = {}; }
  ...
  // --- HELPERS ---
  get allTrackCodes(): string[] { return this.tracks.map(t => t.trackCode); }
  
  highlightText(text: string, term: string): string {
    if (!term) return text;
    const re = new RegExp(`(${term})`, 'gi');
    return text.replace(re, '<span class="text-cyber-primary font-bold border-b border-cyber-primary">$1</span>');
  }
